...
def _resolve_category_from_data(canonical_name: str,
                                staff_ref: Optional[Dict],
                                category_overrides: Dict[str, str]) -> str:
    """Resolve a staff member's contract category, in priority order: Staff
    Categories and FTE.csv, then a previously-saved interactive answer.
    Returns "" if neither covers this person (the caller decides whether to
    ask the user).
    """
    if staff_ref and staff_ref.get("category"):
        return staff_ref["category"]
...
_STAFF_CATEGORIES_HEADER = ["Name", "Category", "FTE", "Modelled", "Notes", "Email", "Teaching % Chart", "Grade"]
...
def _load_staff_categories_and_fte(filepath: str = "Staff Categories and FTE.csv") -> Dict[str, dict]:
    ...
    "teaching_pct_chart": bool, "notes": str, "email": str, "grade": str}}.
    """
    path = DATA_DIR / filepath
    if not path.exists():
        return {}
...
def load_all_data(data_dir: str = None,
    ...
    if hod_name_from_waw and hod_name_from_waw not in staff:
        ...
        staff[hod_name_from_waw] = StaffData(
            ...
            ug_project_load=proj_data_hod["ug_project_load"] if proj_data_hod else 0,
            pg_project_load=proj_data_hod["pg_project_load"] if proj_data_hod else 0,
