...
def _create_individual_staff_report_html(r: WorkloadResult, year_data: YearData) -> str:
    ...
    def _format_module_practicals_section(module_breakdown: Dict[str, Any], css_class: str,
        ...
        if isinstance(practicals_structured, dict) and practicals_structured:
            ...
            # Calculate first session total: one session per week × hours × weeks × rate
            first_session_total = first_session_weekly * config.TEACHING_WEEKS_PER_SEMESTER * first_session_rate
            ...
            parts.append(f"""<div class="detail-item {css_class}" style="padding-left:40px;font-size:0.85em;color:#666;">
                ...
                <span class="detail-hours">{week_count} sessions/week @ {first_session_weekly:.1f}h each = {first_session_total:.1f}h (standard rate)</span>
            </div>""")
